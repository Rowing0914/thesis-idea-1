...
def main(argv):
    ...
    if tf.io.gfile.exists(FLAGS.model_dir):
        tf.compat.v1.logging.warning("Warning: deleting the old log dir at {}".format(FLAGS.model_dir))
        tf.io.gfile.rmtree(FLAGS.model_dir)
    tf.io.gfile.makedirs(FLAGS.model_dir)

    w_true, b_true, x, y = toy_logistic_data(FLAGS.num_examples, 2)
    features, labels = build_input_pipeline(x, y, batch_size=FLAGS.batch_size)
    ...
    neg_log_likelihood = -tf.math.reduce_mean(input_tensor=labels_distribution.log_prob(labels))
    ...
    with tf.compat.v1.name_scope("train"):
        optimiser = tf.compat.v1.train.AdamOptimizer(learning_rate=FLAGS.learning_rate)
        train_op = optimiser.minimize(elbo_loss)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        visualize_decision(x, y, (w_true, b_true), candidate_w_bs, fname="test.png")
