...
if __name__ == '__main__':
    ...
    for episode in range(n_episodes):
        ...
        # == After 1 episode ===
        history = gp_model.fit(agent.Q.flatten()[np.newaxis, ...], np.array([duration]), verbose=False)
        print(history.history)

        pred = gp_model(agent.Q.flatten()[np.newaxis, ...])
        print(pred.sample().numpy().squeeze())
